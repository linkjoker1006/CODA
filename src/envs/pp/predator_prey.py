...
from env.draw import draw_grid, fill_cell, draw_circle, write_cell_text
...
class PredatorPrey(gym.Env):
    ...
    def __init__(self, grid_shape=(5, 5), n_agents=2, n_preys=1, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
        ...
        self.observation_space = MultiAgentObservationSpace(
            [spaces.Box(self._obs_low, self._obs_high, dtype=np.float32) for _ in range(self.n_agents)])
    ...
    def step(self, agents_action):
        ...
        #     self.move_agents = 0
        #     self.stop_agents = 0
        # # 1. 统计移动和停止的智能体数量 (假设动作4为停止)
        # move_agents = 0
        # stop_agents = 0
        # STOP_ACTION_ID = 4  # 根据代码推断，停止动作为4
        # for action in agents_action:
        ...
        # # 2. 计算平均捕获时间
        # num_captured = self.n_preys - sum(self._prey_alive)
        # if num_captured == 0:
        #     avg_capture_time = self._max_steps  # 如果还没有捕获猎物，平均时间为0
        # else:
        #     avg_capture_time = self._step_count / num_captured
        # self.move_agents += move_agents
        # self.stop_agents += stop_agents

        # # 3. 构建包含所有统计信息的info字典
        # info = {
        #     'prey_alive': sum(self._prey_alive),  # 保留原有的信息
        #     'test_info': [
        #         self.move_agents,
        #         self.stop_agents,
        #         avg_capture_time
    ...
    def _entity_not_in_obsrange(self, pos_i, pos_j):
        if abs(pos_i[0] - pos_j[0]) <= (self._agent_view_mask[0] // 2) and abs(pos_i[1] - pos_j[1]) <= (self._agent_view_mask[1] // 2):
            return False
        else:
            return True
    ...
    def render(self, mode='human'):
        ...
        try:
            ...
            # 如果找不到指定的字体文件，就使用一个默认的字体，避免程序报错
        ...
        for agent_i in range(self.n_agents):
            ...
            write_cell_text(img, text=str(agent_i + 1), pos=self.agent_pos[agent_i], cell_size=CELL_SIZE,
                            fill='white', margin=0.4, font=font)
