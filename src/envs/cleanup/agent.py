...
import numpy as np
from .utils import utility_funcs as util
...
class CleanupAgent(Agent):
    def __init__(self, agent_id, start_pos, start_orientation, grid, cleaning_penalty, 
                 reward_value, view_len=CLEANUP_VIEW_SIZE, global_ref_point=None):
    ...
    def fire_beam(self, char):
        if char == 'C':
            self.reward_this_turn += self.cleaning_penalty
    ...
    def consume(self, char):
        """Defines how an agent interacts with the char it is standing on"""
        if char == 'A':
            self.reward_this_turn += self.reward_value
            return ' '
        else:
            return char
