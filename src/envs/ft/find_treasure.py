...
from envs.draw import draw_grid, fill_cell, draw_circle, write_cell_text
...
class FindTreasure(gym.Env):
    ...
    def __init__(self, grid_shape: Coordinates = (8, 8), n_agents: int = 4, n_treasures: int = 1,
                 ...
                 treasure_reward: float = 15, max_steps: int = 50, seed: int = None):

        if not isinstance(grid_shape, tuple):
            grid_shape = tuple(grid_shape)
        if not isinstance(agent_view, tuple):
            agent_view = tuple(agent_view)
        ...
        self.observation_space = MultiAgentObservationSpace([spaces.Box(obs_low, obs_high, dtype=np.float32)] * self.n_agents)
    ...
    def render(self, mode='human'):
        ...
        for pos, agent_strength, sth_level, box_level in self._view_generator(mask):
            ...
            if sth_level != 0:  # item
                ...
                write_cell_text(img, text=str(box.id), pos=sth_pos,
                                cell_size=cell_size, fill='white', margin=0.4, font=font)
        ...
        if mode == 'rgb_array':
            ...
            return self._viewer.render(self, return_rgb_array=True)
    ...
    def _entity_not_in_obsrange(self, pos_i, pos_j):
        if abs(pos_i[0] - pos_j[0]) <= self._agent_view[0] and abs(pos_i[1] - pos_j[1]) <= self._agent_view[1]:
            return False
        else:
            return True
    ...
    #         self.agents_opening_red = 0
    #         self.agents_opening_yellow = 0
    #         self.agents_attempting_pick = 0
    #         self.agents_moving = 0
    #         self.invalid_open_attempts = 0
    #         self.invalid_pick_attempts = 0
    #         self.successful_opens = 0
    #         self.unsuccessful_opens = 0
        
    #     # 对有效目标的动作统计
    #     agents_opening_red = 0
    #     agents_opening_yellow = 0
    #     agents_attempting_pick = 0
    #     agents_moving = 0
    #     # 对无效目标的动作统计
    #     invalid_open_attempts = 0
    #     invalid_pick_attempts = 0
    #     # 对开箱事件的统计
    #     successful_opens = 0
    #     unsuccessful_opens = 0
    #     # ======================================================================
    ...
    #     # ==================== 修改代码：更详细地统计智能体的各类动作 ====================
    #     move_actions = {ACTIONS_IDS['down'], ACTIONS_IDS['left'], ACTIONS_IDS['up'], ACTIONS_IDS['right'], ACTIONS_IDS['noop']}
    #     for agent_id, action in enumerate(agents_action):
    #         agent = self._agents[agent_id] # 获取当前智能体对象

    #         if action in move_actions:
    #             agents_moving += 1

    #         elif action == ACTIONS_IDS['pick']:
    #             # 检查脚下是否有可拾取的物品 (_sth_map > 0)
    #             if self._sth_map[agent.pos] > 0:
    #                 agents_attempting_pick += 1 # 有效的拾取尝试
    #             else:
    #                 invalid_pick_attempts += 1 # 无效的拾取尝试
            
    #         elif action == ACTIONS_IDS['open']:
    #             # 检查脚下是否有未打开的箱子
    #             entity_type, _ = self._get_entity_with_pos(agent.pos)
    #             if entity_type == 3:  # 是未打开的黄箱子
    #                 agents_opening_yellow += 1
    #             elif entity_type == 4:  # 是未打开的红箱子
    #                 agents_opening_red += 1
    #             else: # 在空地、已打开的箱子或物品上执行了open
    #                 invalid_open_attempts += 1
    #     assert agents_moving + agents_opening_red + agents_opening_yellow + agents_attempting_pick + invalid_open_attempts + invalid_pick_attempts == self.n_agents
    #     # ============================================================================
    ...
    #             if len(l_ag) > 0:
    #                 entity, box = self._get_entity_with_pos((pos[0], pos[1]))
    #                 assert box.open and box.something == 1
    #                 rewards += self._sth_map[pos[0], pos[1]] / self.n_agents
    #                 self._sth_map[pos[0], pos[1]] = 0
    #                 box.something = 2

    #     # 开箱事件的逻辑 (这部分只处理对箱子的有效开箱尝试)
    ...
    #         for x, pos in zip(pending, poses):
    #             ag_at_this_grid = np.where(x == 1)[0]
    #             l_ag_opening = list(set(open_ag).intersection(set(ag_at_this_grid)))
    #             if not l_ag_opening:
    #                 continue
    #             if len(ag_at_this_grid) >= 2 and len(l_ag_opening) >= 2:
    #                 successful_opens += 1
    #                 self._box_map[pos[0], pos[1]] = 0
    #                 entity, box = self._get_entity_with_pos(tuple(pos))
    #                 if entity == 3:
    #                     box.open = True
    #                     rewards += self._op_pnt1 / self.n_agents
    #                     self._sth_map[pos[0], pos[1]] = self._coin_reward
    ...
    #                         self._sth_map[pos[0], pos[1]] = self._treasure_reward
    #             else:
    #                 unsuccessful_opens += 1
        
    #     if self._step_count >= self._max_steps:
    #         self._agent_dones = [True] * self.n_agents

    #     # ==================== 修改代码：构建包含所有新统计信息的info字典 ====================
    #     self.agents_opening_red += agents_opening_red
    #     self.agents_opening_yellow += agents_opening_yellow
    #     self.agents_attempting_pick += agents_attempting_pick
    #     self.agents_moving += agents_moving
    #     self.invalid_open_attempts += invalid_open_attempts
    #     self.invalid_pick_attempts += invalid_pick_attempts
    #     self.successful_opens += successful_opens
    #     self.unsuccessful_opens += unsuccessful_opens
    #     info = {
    #         'episode_steps': self._step_count,
    #         'test_info': [
    #             # 对有效目标的动作统计
    #             self.agents_opening_red,       # 尝试开红箱的智能体数
    #             self.agents_opening_yellow,    # 尝试开黄箱的智能体数
    #             self.agents_attempting_pick,   # 尝试拾取有效物品的智能体数
    #             self.agents_moving,            # 执行移动的智能体数
                
    #             # 无效动作统计
    #             self.invalid_open_attempts,    # 在无箱子处开箱的智能体数
    #             self.invalid_pick_attempts,    # 在无物品处拾取的智能体数
                
    #             # 开箱事件结果统计
    #             self.successful_opens,         # 成功开箱的次数
    #             self.unsuccessful_opens        # 失败开箱的次数 (合作人数不够)
    #         ]
    #     }
