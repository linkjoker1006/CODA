...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.name in ["adhoc_td", "cons"]:
                if self.args.name == "cons":
                    self.logger.log_stat("train/negative_weight", self.mac.negative_weight, t_env)
                    self.logger.log_stat("train/positive_weight", self.mac.positive_weight, t_env)
                for i in range(self.mac.n_agents):
                    self.logger.log_stat("train/agent_obs_{}".format(i), len(self.mac.agent_obs[i]), t_env)
                for i in range(self.mac.n_agents):
                    self.logger.log_stat("train/budget_agent_{}".format(i), self.args.budget - self.mac.ask_budget[i], t_env)
